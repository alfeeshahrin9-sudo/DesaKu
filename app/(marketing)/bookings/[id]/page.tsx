...
export default async function BookingConfirmationPage({ params }: Params) {
  ...
  // `bookings` is closed to anon by RLS (migration 0003), so this read runs as
  // the service role. Knowing the booking's UUID is what authorises the view —
  // see the note on submitReview.
  ...
  const { data } = await supabase
    ...
    .select(
      "id, guest_name, check_in, check_out, total_amount, status, experience_ids, " +
        "homestays(price_per_night, villages(name, region)), " +
